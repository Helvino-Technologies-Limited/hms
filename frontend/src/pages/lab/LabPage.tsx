...
import { Plus, FlaskConical, ListOrdered, Pencil } from 'lucide-react';
...
export default function LabPage() {
  ...
  useEffect(() => { setOrdersPage(0); }, [statusFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {tab === 'orders' && (
            <button onClick={() => { setOrderForm({ visitId: '', testId: '' }); setShowOrderModal(true); }}
      ...
      <Modal open={showOrderModal} onClose={() => setShowOrderModal(false)} title="New Lab Order">
        <form onSubmit={handleCreateOrder} className="space-y-4">
          <div>
            ...
            <input type="number" value={orderForm.visitId} onChange={(e) => setOrderForm((p) => ({ ...p, visitId: e.target.value }))}
              placeholder="Enter visit ID" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required />
          </div>
          <div>
