...
type Tab = 'financial' | 'patients' | 'morbidity' | 'moh';
...
export default function ReportsPage() {
  ...
  const loadReport = async () => {
    setLoading(true);
    setReport(null); setMorbidity(null); setMohReport(null);
    try {
      if (tab === 'morbidity') {
...
<p style="font-size:9px;color:#ccc">Developed by Helvino Technologies Limited</p>
  ...
  const tabBtn = (id: Tab, label: string) => (
    <button onClick={() => { setTab(id); setReport(null); setMorbidity(null); setMohReport(null); }}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        {/* Empty state */}
        {!report && !morbidity && !mohReport && !loading && (
